...
public class TaskControl : MonoBehaviour
{
    ...
    public static bool isComplete2 = false;
    ...
    private void FailTask3()
    {
        ...
        if (isclick2)
        {
            ...
            money += 200;
            MySQL sql = new MySQL();
            sql.UpdateDate("20213908_衷茜芝_gamedate", new string[] { "money" }, new string[] { money.ToString() }, "name", PlayerPrefs.GetString("name"));
            PlayerPrefs.SetInt("money", money);
            //PlayerPrefs.SetString("bag", str1);
            sql.Close();
